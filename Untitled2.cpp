...
class periodice{
	public:
              ...
              class periodice1 : public periodice
              {   void out()
              {
                  cout<<"inherited class";
              }
              };
              void periodice::showperiodic()
              {
	...
	fin.close();
			...
			class table1
{
public:char atno[15],name[15],bp[5],mp[5],ec[15],group[5],period[5],block[5],buffer[200];
};
table1 t2[200];
...
void writeRecord()
{
	...
	int k,count;
	app.open("table2.txt",ios::out|ios::app);
	if(!app)
	{
	...
	strcpy(t.buffer,t.atno);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.name);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.bp);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.mp);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.ec);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.group);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.period);
	strcat(t.buffer,"|");
	strcat(t.buffer,t.block);
	

	count=strlen(t.buffer);
	int ts=45-(count);
	cout<<"count"<<ts;
...
printf("atomicno\t\tname\t\tbp\t\tmp\t\tec\n"); 

while(!in.eof())
{
in.getline(t.atno,15,'|');
in.getline(t.name,15,'|');
in.getline(t.bp,5,'|');
in.getline(t.mp,5,'|');
in.getline(t.ec,15,'!');
in.getline(extra,100,'\n');
printf("\n%s\t\t%s\t\t%s\t\t%s\t\t%s",t.atno,t.name,t.bp,t.mp,t.ec); 
I++;
...
in.getline(extra,100,'\n');
//printf("\n%s\t\t%s\t\t%s\t\t%s\t\t%s",t.atno,t.name,t.bp,t.mp,t.ec); 
cout<<t.atno<<"/t"<<t.name<<"/t"<<t.bp<<"/t"<<t.mp<<"/t"<<t.ec;
I++;
...
in.getline(t.bp,5,'|');
in.getline(t.mp,15,'!');
in.getline(extra,1000,'\n');
printf("\n%s\t\t%s\t\t%s\t\t%s\t\t%s",t.atno,t.name,t.ec,t.bp,t.mp); 
I++;
...
{
	fstream in;
	char atno[15],buffer[45],extra[45];
	int I,j;
	table1 t5[200];
	in.open("table2.txt",ios::in);
	if(!in)
	...
	while(!in.eof())
	{
		in.getline(t5[I].atno,15,'|');
		in.getline(t5[I].name,15,'|');
		in.getline(t5[I].bp,5,'|');
		in.getline(t5[I].mp,5,'|');
		in.getline(t5[I].ec,15,'|');
		in.getline(t5[I].group,5,'|');
		in.getline(t5[I].period,5,'|');
		in.getline(t5[I].block,5,'!');
	...
	for(j=0;j<I;j++)
	{
		if(strcmp(atno,t5[j].atno)==0)
		{
			cout<<"\nthe old values of the record witn usn"<<atno<<"are:";
			cout<<"\natno ="<<t5[j].atno;
			cout<<"\nname  ="<<t5[j].name;
			cout<<"\nbp  ="<<t5[j].bp;
			cout<<"\nmp  ="<<t5[j].mp;
	...
	if(j==I)
	{
		cout<<"\nthe record with usn"<<atno<<"is not present";
		return;
	}
	in.close();

	fstream out1;
	out1.open("table2.txt",ios::out);
	...
	for(j=0;j<I;j++)
	{
		strcpy(buffer,t5[j].atno);
		strcat(buffer,"|");
		strcat(buffer,t5[j].name);
		strcat(buffer,"|");
		strcat(buffer,t5[j].bp);
		strcat(buffer,"|");
		strcat(buffer,t5[j].mp);
		strcat(buffer,"|");
		strcat(buffer,t5[j].ec);
		strcat(buffer,"|");
		strcat(buffer,t5[j].group);
		strcat(buffer,"|");
		strcat(buffer,t5[j].period);
		strcat(buffer,"|");
		strcat(buffer,t5[j].block);
...
int main()
{
    ...
    //Initializing atomic numbers.
    {
                    ...
                    strcpy(elem[16].name,"Sulpher- S");          strcpy(elem[16].ec,"[Ne] 3s2 3p4");
...
strcpy(elem[110].origin,"Wilhelm Conrad R�ntgen, physicist");
...
strcpy(elem[114].origin,"IUPAC systematic element name");
    ...
    Sleep(1000);
    cout<<"\t\t\t 3. Add to element"<<"\n\n\n";
    Sleep(1000);
    cout<<"\t\t\t 4. Display element"<<"\n\n\n";
    Sleep(1000);
    cout<<"\t\t\t 5. Modify the file"<<"\n\n\n";
    Sleep(1000);
